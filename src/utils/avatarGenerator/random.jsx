...
function generateSvgAvatar(seed = '', { size = defaultSize, svgNs = defaultSvgNs } = {}) {
...
function genShape(random, color, options) {
  ...
  const sAa =
    minSpreadDist * options.size + random() * Math.max(0, d - minSpreadDist * options.size);
  const sBa =
    minSpreadDist * options.size + random() * Math.max(0, d - minSpreadDist * options.size);
  const sAb =
    minSpreadDist * options.size + random() * Math.max(0, d - minSpreadDist * options.size);
...
class ColorGen {
  ...
  next() {
    ...
    return `rgb(${Math.floor(this.colors[0])},${Math.floor(this.colors[1])},${Math.floor(
      this.colors[2],
    )})`;
  }
}
